...
const THEATRE_URL =
  BASE_URL +
  `/discover/movie?primary_release_date.gte=2020-01-01&primary_release_date.lte=${currentDate()}&` +
  API_KEY;
...
$(document).ready(function () {
  ...
  $(document).mouseup(function (e) {
    let container = $(".movie-list-section");
    if (
      !container.is(e.target) &&
      container.has(e.target).length === 0 &&
      window.innerWidth <= 705
    ) {
      $(container).css("animation", "hide .5s forwards");
      $(container).addClass("hidden");
    } else if ($(container).hasClass("hidden")) {
      $(container).css("animation", "show .5s forwards");
    }
  });
  navigation();
  $("#burgerOpen").on("click", function () {
    $("#burgerMenu").css("width", "270px");
    setTimeout(() => {
      $("#burgerClose").css("display", "block");
      $("#burgerWrapper").css("height", "270px");
    }, 500);
  });
  $("#burgerClose").on("click", function () {
    $("#burgerWrapper").css("height", "0");
    setTimeout(() => {
      $("#burgerMenu").css("width", "0");
    }, 500);
  });
});
...
function getConf() {
  fetch(
    "https://api.themoviedb.org/3/configuration?api_key=da0ec74f280b41c3b79d45fa4cd12578"
  )
    .then((res) => res.json())
    .then((data) => {
      // console.log(data);
...
function getGenres() {
  fetch(
    "https://api.themoviedb.org/3/genre/movie/list?api_key=da0ec74f280b41c3b79d45fa4cd12578&language=en-US"
  )
    .then((res) => res.json())
    .then((data) => {
...
function randomMovie(data) {
  try {
    val = getRandom(0, 20);
    $("body").css({
      background: `radial-gradient(circle, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 100%), url(${
        BACKDROP_URL + data[val].backdrop_path
      })`,
...
async function randomScroll() {
  ...
  for (let e = 0; e < box.length; e++) {
    box[e].addEventListener("click", function () {
      ...
      $(".stars").attr(
        "style",
        `--rating: ${data.results[e].vote_average / 2};`
      );
...
function getVideo(data) {
  fetch(`${BASE_URL}/movie/${data.results[val].id}/videos?${API_KEY}`)
    .then((res) => res.json())
    .then((data) => {
      if (data) {
        try {
...
async function getMovQuerries(page) {
  ...
  for (let i = 0; i < fbox.length; i++) {
    fbox[i].addEventListener("click", function () {
      ...
      fetch(
        `${BASE_URL}/movie/popular?${API_KEY}&page=${3}&with_genres=${filterarr.join(
          ","
        )}`
      )
        .then((res) => res.json())
        .then((data) => {
          ...
          if (data.results.length != 0) {
            for (let i = 0; i < 3; i++) {
              $(".movie-quarry").append(
                '<div class="movie-wrapper-flex"></div>'
              );
            }
            for (let d = 0; d < data.results.length; d++) {
              if (data.results[d].title.length > 21) {
                data.results[d].title = `${data.results[d].title.slice(
                  0,
                  20
                )}...`;
              }
              generateShows(
                data.results[d].title,
                data.results[d].backdrop_path,
                "movie-wrapper-flex",
                data.results[d].overview
              );
            }
            $(".about").on("click", function () {
              $(this)
...
function loadFav(arr) {
  $(".main-section").append(`<div class="shows-main-grid"></div>`);
  Object.values(arr[0]).forEach((e) => {
    $(".shows-main-grid").append(`
      <div class="show-block">
        <div class="img-container">
          <img src="${JSON.parse(e).img}">
        </div>
        <div class="title-space">
          <h6>${JSON.parse(e).header}</h6>
          <div class="settings">
          <div class="settings-menu">
            ...
            <ul>
                <li class="li-rff"><span>Remove from favourites</span></li>
            </ul>
          </div>
          •••
          </div>
  ...
  [...removers].forEach((e, i) => {
    e.addEventListener("click", function () {
      Object.values(arr[0]).forEach((y) => {
        if (
          $(this).parentsUntil(".show-block").find("h6").text() ==
          JSON.parse(y).header
        ) {
          Object.keys(localStorage).find((k) => {
            if (
              localStorage[k].includes(
                $(this).parentsUntil(".show-block").find("h6").text()
              )
            ) {
              localStorage.removeItem(k);
              $(e).parents().find($(".show-block"))[i].remove();
            }
          });
        }
      });
...
`);
  ...
  Object.values(arr[0]).forEach((e) => {
    if (JSON.parse(e).header == n) {
      $(`.show-block:contains('${n}')`)
        .find(".li-atf")
